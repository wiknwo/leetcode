'''Design LinkedList (Singly LinkedList)
    ...
    node in the linked list. Assume all nodes in the linked 
    list are 0-indexed.
    ...
    - void addAtHead(int val) Add a node of value val 
    before the first element of the linked list. After the 
    insertion, the new node will be the first node of the 
    linked list.

    - void addAtTail(int val) Append a node of value val as 
    the last element of the linked list.

    - void addAtIndex(int index, int val) Add a node of 
    ...
    - void deleteAtIndex(int index) Delete the indexth node 
...
class MyLinkedList:
    ...
    def addAtTail(self, val: int) -> None:
        ...
        """
        if self.head == None:
            self.head = newnode = ListNode(val)
            self.length += 1
        else:
            current = self.head
            while current.next:
